...
import { Upload, Link2, Globe, RefreshCw, X, FileText, BookOpen, ExternalLink, ChevronDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { AREAS, SUGGESTED_DOCS, AREA_MAP } from "../_lib/constants";
...
export default function UploadPanel({ onClose, processing, onIndexUrl, onUploadFile, indexedFilenames, pypdfConfirm, onPypdfConfirm, onPypdfCancel }) {
        ...
        <div className="p-5">
          <Tabs defaultValue="suggested">
            <TabsList className="mb-4">
              <TabsTrigger value="suggested"><BookOpen className="h-3.5 w-3.5 mr-1.5" /> Sugeridos</TabsTrigger>
            ...
            <TabsContent value="suggested">
              <div className="space-y-2">
                {SUGGESTED_DOCS.map((doc, idx) => {
                  ...
                  const currentUrl = editedUrls[idx] !== undefined ? editedUrls[idx] : doc.url;
                    ...
                    <div key={doc.filename} className={`rounded-lg border transition-colors ${isIndexed ? "bg-green-500/5 border-green-500/20" : "hover:bg-muted/20"}`}>
                      <div className="flex items-center gap-3 p-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">{doc.pages} págs · {doc.size}</p>
                        </div>
                        ...
                        <button onClick={() => setExpandedDoc(isExpanded ? null : idx)} className="p-1 hover:bg-muted rounded">
                      ...
                      {isExpanded && (
                        <div className="px-3 pb-3 border-t mx-3 pt-2">
                          <div className="flex gap-2">
                            <input type="url" value={currentUrl} onChange={(e) => setEditedUrls({ ...editedUrls, [idx]: e.target.value })} className="flex-1 px-2.5 py-1.5 bg-muted border rounded-md text-xs font-mono focus:border-primary focus:outline-none" />
                  ...
                  <div className="space-y-2">
                    {selectedFiles.map((f, i) => (
                      <div key={i} className="flex items-center gap-3 p-2 rounded-lg border">
                        ...
                        <button onClick={() => setSelectedFiles((prev) => prev.filter((_, j) => j !== i))} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-red-500">
